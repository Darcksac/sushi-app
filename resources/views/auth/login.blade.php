...
<head>
    ...
    <title>Iniciar Sesión - SushiPekiteki</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="form-container">
        ...
        <form class="form" method="POST" action="{{ route('login') }}">
            ...
            <input type="email" name="email" class="input @error('email') input-error @enderror" placeholder="Email" value="{{ old('email') }}" required autofocus>
            @error('email')
                <div class="error-message">{{ $message }}</div>
            @enderror
            
            <input type="password" name="password" class="input @error('password') input-error @enderror" placeholder="Password" required>
            @error('password')
                <div class="error-message">{{ $message }}</div>
            @enderror
            ...
            <p class="page-link">
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" style="color: inherit; text-decoration: none;">
        ...
        <p class="sign-up-label">
            Don't have an account?<a href="{{ route('register') }}" style="text-decoration: none;"><span class="sign-up-link">Sign up</span></a>
        </p>
        ...
        <div class="buttons-container">
            ...
            <div class="google-login-button" onclick="loginWithGoogle()" style="cursor: pointer;">
...
<script type="module">
  ...
  window.loginWithGoogle = function() {
      if (!app) {
          alert("Firebase no está configurado en el archivo .env");
          return;
      }
      ...
      signInWithPopup(auth, provider)
        ...
        .then((idToken) => {
            return fetch("{{ route('auth.firebase') }}", {
                method: "POST",
                headers: {
        ...
        .catch((error) => {
            console.error(error);
            alert("Error con Google Login: " + error.message);
        });
  }
</script>
